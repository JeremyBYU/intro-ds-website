...
- Was there anyting special about the observations in test/validation data set?
- It seems a little strange to now judge all the models just on the random selection of observations of validation/test sets. 
- Its like the choice of our model is *biased* towards this random selection of data.
- Solution: Do this process (fitting a model, testing model) *multiple* times using different subsets of data -> Cross Validation
- For each model trained/validated, a measure of error is used to measure performance. The distribution of the errors for the models found in cross-validation gives a rough idea of the model's performance overall.
